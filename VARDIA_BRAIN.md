...
- **Αντιγραφή σε όλη την εβδομάδα**: **ΜΟΝΟ long-press 450ms** (ή δεξί κλικ σε desktop) σε γεμάτο κελί → confirm → αντιγραφή σε όλη τη γραμμή. ΟΧΙ drag & drop (απορρίφθηκε για mobile, UI spec §5.1). ~~Toggle «Όλη η εβδομάδα» στο pad~~ — **αφαιρέθηκε 2026-08-21 (Φώτης)**: δύο τρόποι για το ίδιο πράγμα = περιττή πολυπλοκότητα, και το toggle εισήγαγε κρυφή κατάσταση που ο owner μπορεί να ξεχάσει ενεργή. Το hint στο pad κάνει το long-press ανακαλύψιμο.
